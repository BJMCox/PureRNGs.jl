...
function measure_fill(rng, values, seconds, threaded)
    ...
    rng, _ = rand_next!(rng, values; threaded)
...
result = measure_fill(rng, values, seconds, threaded)

@printf(
    "%s %s on %s: %.3f Gvalue/s, %.3f GiB/s output, %.3f ms/fill (%d runs)\n",
    nameof(family),
    result_type,
