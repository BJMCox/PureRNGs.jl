...
@testset "bridge failure stability" begin
    for (T, width, draw) in (
        ...
        (Float32, IR._exponential_bits(Float32), randexp),
        (Float64, IR._exponential_bits(Float64), randexp),
    )
...
@testset "owned bridge fills" begin
    ...
    exponential_last = _bridge_last(Philox2x32(0x816), IR._exponential_bits(Float64))
...
@testset "foreign bridge fills" begin
    ...
    exponential_root = _bridge_last(Philox2x32(0x817), IR._exponential_bits(Float64))
